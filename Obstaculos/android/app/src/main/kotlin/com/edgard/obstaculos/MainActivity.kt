...
class MainActivity : FlutterActivity() {
    ...
    private fun initSounds() {
        ...
        val names = listOf(
            "walk", "fail", "cheer", "destroy", "climb", "splash", "whoosh", "tick",
        )
        for (name in names) {
            val resId = resources.getIdentifier(name, "raw", packageName)
            if (resId != 0) {
                soundIds[name] = pool.load(this, resId, 1)
            }
        }
        soundPool = pool
    }
    ...
    companion object {
        private const val SYSTEM_UI_CHANNEL = "com.edgard.obstaculos/system_ui"
        private const val AUDIO_CHANNEL = "com.edgard.obstaculos/audio"
    }
}
