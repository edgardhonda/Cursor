...
class _GameScreenState extends State<GameScreen>
  ...
  static const _systemUiChannel =
      MethodChannel('com.defendercastelo.defender_castelo/system_ui');
  ...
  void initState() {
    ...
    _ticker = createTicker((_) {
      _engine.tick();
      if (_engine.castleDestroyed != _showRestart) {
        setState(() => _showRestart = _engine.castleDestroyed);
      }
    });
    _ticker.start();
  }
              ...
              builder: (context, constraints) {
                ...
                return Column(
                  children: [
                          ...
                          children: [
                            Positioned.fill(
                              child: Listener(
                                ...
                                onPointerDown: (e) => _engine.pointerDown(
                                    e.pointer, e.localPosition),
                                onPointerMove: (e) => _engine.pointerMove(
                                    e.pointer, e.localPosition),
                                onPointerUp: (e) => _engine.pointerUp(
                                    e.pointer, e.localPosition),
                                onPointerCancel: (e) => _engine.pointerUp(
                                    e.pointer, e.localPosition),
                                child: CustomPaint(
                    ...
                    _ControlBar(pinned: _pinned, onTogglePin: _onTogglePin),
                  ],
                );
              },
            ),
...
class _ControlBar extends StatelessWidget {
  const _ControlBar({required this.pinned, required this.onTogglePin});
  ...
  Widget build(BuildContext context) {
    ...
    final btnHeight = isWide ? 48.0 : 42.0;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          const Text(
            'Toque a faixa → cria soldado   •   Toque o castelo → dispara',
